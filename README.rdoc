...
To run, Download and Unzip the file, and within the document, enter the command "rake db:migrate RAILS_ENV=development", then run "rails s" or "rails server". This starts the server. Then in a web browser, enter "http://localhost:3000" or "http://127.0.0.1:3000" to connect to your computers local address on port 3000 (The default address for Ruby on Rails server). The home page shows up where it prompts you to create an account or login. You can create an account yourself, or use the username "a" and password "a" to access the root account, which can access a page that shows all users. There are a few other dummy accounts and a few objects for "sale" created by those accounts. 
